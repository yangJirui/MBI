...
def save_combined_res(path):
    mbi = np.load('../data/res/mbi_raw.npy')
    mbi = np.array(mbi, dtype=np.uint8)
    ...
    combined_img = np.concatenate((raw_img, np.expand_dims(mbi, axis=0)), axis=0)

    # 创建文件
    driver = gdal.GetDriverByName("GTiff")  # 数据类型必须有，因为要计算需要多大内存空间
    dataset = driver.Create('../data/res/combined_res.tif', 650, 650, 9, gdal.GDT_UInt16)
...
if __name__ == '__main__':
    # with_threshold()
    save_combined_res('../data/Four_Vegas_img96.tif')